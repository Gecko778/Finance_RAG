...
## 2026-07-27 · 🎯 端到端全链路打通（key 就绪后一次性关闭 M2/M3/M5/M6/M7 待办）

用户配好 **SiliconFlow 中国站 key（.cn，BGE-M3+reranker 免费）+ DeepSeek key**，一次性真实跑通此前所有 ⏳：

| 环节 | 结果 |
|---|---|
| **M2 ready** | 上传 3 份真实政策 → 全部 `ready`（起征点公告 9 块 / 增值税法 40 块 / 实施条例 91 块），Qdrant 共 **140 向量**；清理了早期无效 key 的残留失败记录 |
| **M3 /retrieval** | 真实 embedding+rerank，返回带出处片段（score/文号/条款序号） |
| **M3 /chat** | DeepSeek 流式生成：起征点问题答"季度30万元[1]"带引用+复核提示；库外问题（企业所得税）**正确拒答不编造** |
| **M5 前端对话** | 浏览器实操：登录→提问→流式回答→**引用了 docx 表格行**（M2 最大风险"表格解析"端到端验证通过）；反馈按钮点击→后端入库 |
| **M6 评测** | run_eval 升级为 /chat 级评测；自动基线 **5/8**，人工核验 **8/8 实质正确**（3 条"失败"为子串判据假阴性，答案实际正确，已存证 eval_result.json）；反馈链路前端→DB 打通 |
| **M7** | 插件契约此前已验证；语音需机器人硬件环境，非软件问题 |

**波折排查**：用户首个 key 是 SiliconFlow **国际站(.com)**（模型目录不同、按量付费余额不足），排查后改用**中国站(.cn)**（免费 BGE 系列、零代码改动）解决。

**关键结论**：数据库→后端→前端→检索→生成→引用→反馈**全链路真实可用**；财税防幻觉（拒答）、政策表格解析、引用溯源均验证通过。
...
**K8s 真集群运行时验证（补）**：kind 节点镜像(~1GB)本环境拉取超时→改 **k3d** 起真集群，全栈部署（2×api+2×web+worker+4 数据服务）+ migrate Job（迁移/种子/Qdrant 初始化）+ 集群内上传→ready→/chat 问答"季度30万元[1]"**全部跑通**（12 项，详见 test.md）；验证后删除集群释放资源。

...
**任务方向**：
- [x] 部署基础设施：api / worker 容器化（Dockerfile），配合前端 Dockerfile → 三镜像齐备（已验证可构建可导入）
- [x] K8s 编排：`k8s/`（namespace/config/secret 模板/数据服务 StatefulSet/api·worker·web/migrate-job/ingress，kustomize）— 结构校验通过（18 资源）；默认集群内有状态服务、可外置托管
- [x] 数据合规文档 + 两种交付形态说明：`docs/compliance-and-delivery.md`（数据流向、切本地模型路径、共享 SaaS vs 私有化、人工复核硬规）
- [ ] 租户自助开通流程（注册→建租户→初始配额）— **待产品决策（公开注册 vs 管理员开通）**
- [ ] 用量计量钩子 + 计费定价模型 — **待产品决策**
- [x] K8s 运行时部署验证（2026-07-27，k3d 真集群）：全栈部署+迁移种子+集群内问答"季度30万元[1]"跑通（kind 节点镜像本环境拉不下，改 k3d）

**说明**：K8s 清单 + 合规/交付文档已完成并结构校验；租户自助注册与计费定价属产品决策，按 plan"另行详细规划"保留待负责人拍板，不臆想。
...
| 2026-07-27 | M8 K8s验证 | k3d 真集群端到端部署验证：全栈 Running+迁移种子+集群内 /chat 问答跑通 | 12 项通过（kind 节点镜像本环境拉不下，改 k3d，已删测试集群） | test(k8s) |
| 2026-07-27 | M8(部分2) | K8s 部署清单(kustomize 18 资源)+合规/交付文档；租户自助注册·计费待产品决策 | kustomize 结构校验通过；运行时部署待真集群 | feat(M8) |

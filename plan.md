...
- [ ] **对话问答 E2E**：待 SiliconFlow + DeepSeek key（与 M2/M3 一起补）

**验收**：登录→建库→上传（表单）→提问→带出处回答 → **✅ 部分通过（2026-07-27）：前 4 页构建+浏览器渲染验证（真实登录/数据/RBAC UI）+ 后端接口实测；问答 E2E 待 key（test.md #15）**
...
- [x] 内测反馈渠道：feedback 表 + POST/GET 接口 + 前端"有帮助/不准确"按钮
- [ ] 组织财税人员真人内测：真实政策库灌入 + 收集 badcase（**待 key + 测试人员**）
- [ ] RAGAS 基线 + badcase 归因迭代（**待 key**）

**验收**：评测集跑通并有基线报告；高频 badcase 有归因 → **⏳ 基础设施就绪并实测（2026-07-27，10 项通过）；真实跑分/真人内测待 key（test.md #11-13）**
